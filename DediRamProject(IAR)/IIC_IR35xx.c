...
static void PROCESS_DECLARE(command_initchip)
{
	...
	sram[VOFS_COMMAND] = VCMD_INIT_CHIP;
        ...
        unsigned int slave;
        read_sram(0x25,&slave,1);
        TraceString("slave=");TraceInt(slave);TraceString("\r\n");
}
...
static void PROCESS_DECLARE(command_verify)
{
	...
	sram[VOFS_PAGE_SIZE] = info->uiPageSizeInByte;
        ...
        unsigned int pVcc = info->uiUnlockParameter;
        if(info->uiUnlockParameter == 1)
        {
            int x=2000000;
            CheckStatus(50000);
            VectorSetSocketVcc(vector->cpCallbacks,OFF);
            VectorSetSocketVpp(vector->cpCallbacks,OFF);    
            while(x>0)
              {x--;}//200ms
            VectorSetSocketVcc(vector->cpCallbacks,ON);
            VectorSetSocketVpp(vector->cpCallbacks,ON);
        }
        TraceString("pVcc=");TraceInt(pVcc);TraceString("\r\n");
	TraceString("VOFS_START_ADDRESS=");TraceInt(sram[VOFS_START_ADDRESS]);TraceString("\r\n");
        TraceString("VOFS_LENGTH=");TraceInt(sram[VOFS_LENGTH]);TraceString("\r\n");
        TraceString("VOFS_PAGE_SIZE=");TraceInt(sram[VOFS_PAGE_SIZE]);TraceString("\r\n");
...
static void PROCESS_DECLARE(command_program)
{
	...
	sram[VOFS_LAST_SUCCESS] = vector->uiStartPage;
	sram[VOFS_LENGTH] = vector->uiImageLength/info->uiPageSizeInByte;
	sram[VOFS_PAGE_SIZE] = info->uiPageSizeInByte ; //pageSize = 64Bytes
        
        //Only for PartNumber_GigabytesOnly
        unsigned int pSkip = info->uiInitParameter;
        unsigned int ReadSum = 0;
        unsigned int DFIFOSum = 0;
        
        read_sram(0x31,&DFIFOSum,1);
        read_sram(0x32,&ReadSum,1);
        write_sram(0x33,&pSkip,1);
        
        TraceString("DFIFOSum=");TraceInt(DFIFOSum);TraceString("\r\n");
        TraceString("ReadSum=");TraceInt(ReadSum);TraceString("\r\n");
        TraceString("pSkip=");TraceInt(pSkip);TraceString("\r\n");
