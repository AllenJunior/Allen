...
unsigned int defaultvalue[] = {
         //Address
        0x0660,0x6021,0x0009,0x3F00,0x0000,0x0000,0x3203,0x3207,
	0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,
	0x1212,0x1212,0x1212,0x1212,0x0000,0x0000,0x0000,0x0000,
	0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,
	0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,
};
...
static void PROCESS_DECLARE(command_erase)
{
	...
	sram[VOFS_LENGTH] = vector->uiImageLength/info->uiPageSizeInByte;
	sram[VOFS_PAGE_SIZE] = info->uiPageSizeInByte;
        write_sram(0x30,(unsigned int*)defaultvalue,sizeof(defaultvalue));
        
        /*
        unsigned int i;
        for (unsigned int sram = 0x30;sram<0x57 ; sram++)
        {
          read_sram(sram,&i,1);
          TraceInt(sram);TraceString("=");TraceInt(i);TraceString("\r\n");
        }
        */
        //TraceString("VOFS_START_ADDRESS=");TraceInt(sram[VOFS_START_ADDRESS]);TraceString("\r\n");
        TraceString("VOFS_PAGE_SIZE=");TraceInt(sram[VOFS_PAGE_SIZE]);TraceString("\r\n");
...
static void PROCESS_DECLARE(command_blankcheck)
{
	...
	sram[VOFS_PAGE_SIZE] = info->uiPageSizeInByte;
        ...
        /*
        unsigned int i;
        for (unsigned int Bsram = 0x30;Bsram<0x57 ; Bsram++)
        {
          read_sram(Bsram,&i,1);
          TraceInt(Bsram);TraceString("=");TraceInt(i);TraceString("\r\n");
        }
        */
        //TraceString("VOFS_START_ADDRESS=");TraceInt(sram[VOFS_START_ADDRESS]);TraceString("\r\n");
        TraceString("VOFS_PAGE_SIZE=");TraceInt(sram[VOFS_PAGE_SIZE]);TraceString("\r\n");
