...
static void PROCESS_DECLARE(command_initchip)
{
	...
	TraceString("Initial\r\n");
        write_sram(0x20,&info->uiInitParameter,1);      //SRAM[12] = Preload data counter
	sram[VOFS_COMMAND] = VCMD_INIT_CHIP;
        
        unsigned int i ;
        read_sram(0x20,&i,1);
        TraceString("Preload=");TraceInt(i);TraceString("\r\n");
}
...
static void PROCESS_DECLARE(command_read)
{
	...
	sram[VOFS_START_ADDRESS] = vector->uiStartPage; //*(info->uiPageSizeInByte/2) + vector->usOffsetInPage;
        sram[VOFS_LENGTH] = vector->uiImageLength / info->uiPageSizeInByte ;
        sram[VOFS_PAGE_SIZE] = info->uiPageSizeInByte /2 ;
        write_sram(0x21,&info->uiReadParameter,1);      //SRAM[0x21] = Addr_Length data counter
        write_sram(0x22,&info->uiProgramParameter,1);   //SRAM[0x22] = Last 2 bit counter
...
static void PROCESS_DECLARE(command_blankcheck)
{
	...
	sram[VOFS_START_ADDRESS] = vector->uiStartPage;
...
static void PROCESS_DECLARE(command_verify)
{
	...
	sram[VOFS_LAST_SUCCESS] = vector->uiStartPage;
        ...
        write_sram(0x21,&info->uiReadParameter,1);      //SRAM[0x21] = Addr_Length data counter
        write_sram(0x22,&info->uiProgramParameter,1);   //SRAM[0x22] = Last 2 bit counter
        write_sram(0x23,vector->vpParameter,vector->uiParameter/4); 
...
static void PROCESS_DECLARE(command_program)
{
	...
	sram[VOFS_LAST_SUCCESS] = vector->uiStartPage;
        ...
        int c,d,e,f,g;
        read_sram(0x23,&c,1);
        TraceString("SRAM[23]=");TraceInt(c);TraceString("\r\n");
        read_sram(0x24,&d,1);
        TraceString("SRAM[24]=");TraceInt(d);TraceString("\r\n");
        read_sram(0x25,&e,1);
        TraceString("SRAM[25]=");TraceInt(e);TraceString("\r\n");
        read_sram(0x26,&f,1);
        TraceString("SRAM[26]=");TraceInt(f);TraceString("\r\n");
        read_sram(0x27,&g,1);
        TraceString("SRAM[27]=");TraceInt(g);TraceString("\r\n");
        */
        TraceString("VOFS_START_ADDRESS=");TraceInt(sram[VOFS_START_ADDRESS]);TraceString("\r\n");
        TraceString("VOFS_LENGTH=");TraceInt(sram[VOFS_LENGTH]);TraceString("\r\n");
        TraceString("VOFS_PAGE_SIZE=");TraceInt(sram[VOFS_PAGE_SIZE]);TraceString("\r\n");
