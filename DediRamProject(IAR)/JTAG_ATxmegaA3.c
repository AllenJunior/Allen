...
{
	const IC_INFO* info = (const IC_INFO*)vector->cvpIcInformation;
        unsigned int startpage = vector->uiStartPage;
        
        TraceString("Read\r\n");
	sram[VOFS_COMMAND] = VCMD_READ;
        sram[VOFS_OP_AREA] = vector-> usOperateArea;
        if(vector->usOperateArea == 2)
        {
          //Series Else
          //sram[VOFS_START_ADDRESS] = vector->uiStartPage*(info->uiPageSizeInByte/8)+ vector->usOffsetInPage;
          //sram[VOFS_LENGTH] = vector->uiImageLength /(info->uiPageSizeInByte /8);
          //sram[VOFS_PAGE_SIZE] = info->uiPageSizeInByte /8 /2 ;
          //SeriesE
          sram[VOFS_START_ADDRESS] = vector->uiStartPage*(info->uiPageSizeInByte/4)+ vector->usOffsetInPage;
          sram[VOFS_LENGTH] = vector->uiImageLength /(info->uiPageSizeInByte /4);
          sram[VOFS_PAGE_SIZE] = info->uiPageSizeInByte /4 /2 ;
...
static void PROCESS_DECLARE(command_verify)
{
	...
	TraceString("Verify\r\n");
        ...
        sram[VOFS_OP_AREA] = vector-> usOperateArea;
        write_sram(0x21,vector->vpParameter,vector->uiParameter/4); 
        if(vector->usOperateArea == 2)
        {
          //sram[VOFS_START_ADDRESS] = vector->uiStartPage*(info->uiPageSizeInByte/8)+ vector->usOffsetInPage;
          //sram[VOFS_LENGTH] = vector->uiImageLength /(info->uiPageSizeInByte /8);
...
static void PROCESS_DECLARE(command_program)
{
	...
	sram[VOFS_COMMAND] = VCMD_PROGRAM;
        ...
        read_sram(0x21,&i,1);
        read_sram(0x22,&j,1);
        TraceString("0x21=");TraceInt(i);TraceString("\r\n");
        TraceString("0x22=");TraceInt(j);TraceString("\r\n");
        TraceString("VOFS_START_ADDRESS=");TraceInt(sram[VOFS_START_ADDRESS]);TraceString("\r\n");
        TraceString("VOFS_PAGE_SIZE=");TraceInt(sram[VOFS_PAGE_SIZE]);TraceString("\r\n");
        TraceString("VOFS_LENGTH=");TraceInt(sram[VOFS_LENGTH]);TraceString("\r\n");
        //TraceString("VOFS_LAST_SUCCESS=");TraceInt(sram[VOFS_LAST_SUCCESS]);TraceString("\r\n");
