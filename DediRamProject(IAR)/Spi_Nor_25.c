...
static void PROCESS_DECLARE(command_read)
{
	const IC_INFO* info = (const IC_INFO*)vector->cvpIcInformation;
 	...
 	TraceString("startpage=");TraceHex(vector->uiStartPage);TraceString("\r\n");
 	TraceString("PageSizeInByte=");TraceHex(info->uiPageSizeInByte);TraceString("\r\n");
 	TraceString("OffsetInPage=");TraceHex(vector->usOffsetInPage);TraceString("\r\n");    
 	TraceString("ImageLength=");TraceHex(vector->uiImageLength);TraceString("\r\n");        
...
const VECTOR_FUNC vector_func[16] = {
	...
	{command_readid,NULL},//VCMD_READ_ID			3
