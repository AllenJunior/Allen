...
static void PROCESS_DECLARE(command_initchip)
{
	...
	sram[VOFS_COMMAND] = VCMD_INIT_CHIP;
        unsigned int ttagspeed = info->uiInitParameter;
        write_sram(0x17,&ttagspeed,1);
}
...
static void PROCESS_DECLARE(command_read)
{
	...
	sram[VOFS_PAGE_SIZE] = info->uiPageSizeInByte;
        ...
        read_sram(0x23,&k,1);
        read_sram(0x24,&l,1);
        TraceString("SRAM[0x21]=");TraceInt(i);TraceString("\r\n");
        TraceString("SRAM[0x22]=");TraceInt(j);TraceString("\r\n");
        TraceString("SRAM[0x23]=");TraceInt(k);TraceString("\r\n");
        TraceString("SRAM[0x24]=");TraceInt(l);TraceString("\r\n");
	*/
...
static void PROCESS_DECLARE(command_erase)
{
	...
	//sram[VOFS_LENGTH] = vector->uiImageLength/info->uiPageSizeInByte;
	//sram[VOFS_PAGE_SIZE] = info->uiPageSizeInByte;
        sram[VOFS_LENGTH] = vector->uiImageLength/info->uiSectorSizeInByte;
        sram[VOFS_PAGE_SIZE]= info->uiSectorSizeInByte;
