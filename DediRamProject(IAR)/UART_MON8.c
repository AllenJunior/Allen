...
static void PROCESS_DECLARE(command_initchip)
{
	...
	TraceString("Initial\r\n");
        ...
        unsigned int MONEN = info->uiInitParameter;
        write_sram(0x17,&MONEN,1);
...
static void PROCESS_DECLARE(command_read)
{
	...
	sram[VOFS_COMMAND] = VCMD_READ;
        ...
        if((vector->usOperateArea == 0) || (vector->usOperateArea == 3))
        {
        ...
        {
          sram[VOFS_START_ADDRESS] = vector->uiStartPage*(info->uiPageSizeInByte/64);
          sram[VOFS_LAST_SUCCESS] = vector->uiStartPage;
          sram[VOFS_LENGTH] = vector->uiImageLength/(info->uiPageSizeInByte/64);
          sram[VOFS_PAGE_SIZE] = info->uiPageSizeInByte/64;
        }
        /*
...
static void PROCESS_DECLARE(command_verify)
{
	...
	if((vector->usOperateArea == 0) || (vector->usOperateArea == 3))
        ...
        write_sram(0x21,vector->vpParameter,vector->uiParameter/4); //Option Bytes
