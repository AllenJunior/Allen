...
namespace numeric_conversion
{
    ...
    unsigned long long string_to_size_t(const std::basic_string<T>& s)
    {
    ...
    template <typename T>
    unsigned long long hexstring_to_size_t(const std::basic_string<T>& s)
    {
        return string_to_size_t<T, true>(s);
    }

    template <typename T>
    unsigned long long hexstring_to_size_t(const T* s)
    {
