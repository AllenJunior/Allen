// Dlg_ATxmegaA.cpp : implementation file
//

#include "stdafx.h"
#include "Dlg_ATxmegaA.h"
#include "numeric_conversion.h"
#include "../common/prog_info.h"

// Dlg_ATxmegaA dialog

IMPLEMENT_DYNAMIC(CDlg_ATxmegaA, CMFCPropertyPage)

CDlg_ATxmegaA::CDlg_ATxmegaA(struct chip_info_c *ChipInfo, unsigned char *RegisterBuff, unsigned long BuffLen, unsigned long *pDataLen, CWnd* pParent /*=NULL*/)
: CMFCPropertyPage(CDlg_ATxmegaA::IDD),
m_ChipInfo(ChipInfo),
...
CDlg_ATxmegaA::~CDlg_ATxmegaA()
{
}

void CDlg_ATxmegaA::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO8, m_JTAGEN);
...
BEGIN_MESSAGE_MAP(CDlg_ATxmegaA, CMFCPropertyPage)
	ON_CBN_SELCHANGE(IDC_COMBO1, &CDlg_ATxmegaA::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO2, &CDlg_ATxmegaA::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO3, &CDlg_ATxmegaA::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO4, &CDlg_ATxmegaA::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO5, &CDlg_ATxmegaA::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO6, &CDlg_ATxmegaA::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO7, &CDlg_ATxmegaA::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO8, &CDlg_ATxmegaA::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO9, &CDlg_ATxmegaA::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO10, &CDlg_ATxmegaA::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO11, &CDlg_ATxmegaA::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO12, &CDlg_ATxmegaA::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO13, &CDlg_ATxmegaA::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO14, &CDlg_ATxmegaA::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO15, &CDlg_ATxmegaA::OnCbnSelchangeCombo)
	ON_EN_CHANGE(IDC_EDIT1, &CDlg_ATxmegaA::OnEnChangeJTAGUID)
	ON_BN_CLICKED(IDC_CHECK1, &CDlg_ATxmegaA::OnBnClickedEnable)
END_MESSAGE_MAP()


// Dlg_ATxmegaA message handlers
BOOL CDlg_ATxmegaA::OnInitDialog()
{
...
BOOL CDlg_ATxmegaA::OnSetActive()
{
	if (*m_pDataLen != NULL)
	{
...
void CDlg_ATxmegaA::OnCbnSelchangeCombo()
{
	CString Cfg1;
	CString Cfg2;
	CString Cfg4;
...
LRESULT CDlg_ATxmegaA::OnWizardNext()
{
	OnOK();
	return CMFCPropertyPage::OnWizardNext();

}
LRESULT CDlg_ATxmegaA::OnWizardBack()
{
	OnOK();
	return CMFCPropertyPage::OnWizardBack();
}
void CDlg_ATxmegaA::OnEnChangeJTAGUID()
{
	CString Cfg0;
	//Fuse0 = 0xFF;
...
void CDlg_ATxmegaA::OnBnClickedEnable()
{
	CButton* Encheck = (CButton*)GetDlgItem(IDC_CHECK1);
	int state = Encheck->GetCheck();
	if (state == 0x1)
