// Dlg_ATxmegaBCD.cpp : implementation file
//

#include "stdafx.h"
#include "Dlg_ATxmegaBCD.h"
...
IMPLEMENT_DYNAMIC(CDlg_ATxmegaBCD, CMFCPropertyPage)

CDlg_ATxmegaBCD::CDlg_ATxmegaBCD(struct chip_info_c *ChipInfo, unsigned char *RegisterBuff, unsigned long BuffLen, unsigned long *pDataLen, CWnd* pParent /*=NULL*/)
: CMFCPropertyPage(CDlg_ATxmegaBCD::IDD),
m_ChipInfo(ChipInfo),
...
CDlg_ATxmegaBCD::~CDlg_ATxmegaBCD()
{
}

void CDlg_ATxmegaBCD::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO9, m_BODACT);
	DDX_Control(pDX, IDC_COMBO10, m_EESAVE);
	DDX_Control(pDX, IDC_COMBO11, m_BODLEVEL);
	DDX_Control(pDX, IDC_COMBO12, m_BLBB);
	DDX_Control(pDX, IDC_COMBO13, m_BLBA);
	DDX_Control(pDX, IDC_COMBO14, m_BLBAT);
	DDX_Control(pDX, IDC_COMBO15, m_LB);
	DDX_Control(pDX, IDC_CHECK1, Option_Enable);
	DDX_Control(pDX, IDC_EDIT1, m_Cfg1);
	DDX_Control(pDX, IDC_EDIT2, m_Cfg2);
	DDX_Control(pDX, IDC_EDIT3, m_Cfg4);
	DDX_Control(pDX, IDC_EDIT4, m_Cfg5);
	DDX_Control(pDX, IDC_EDIT5, m_Cfg6);
}


BEGIN_MESSAGE_MAP(CDlg_ATxmegaBCD, CMFCPropertyPage)
	ON_CBN_SELCHANGE(IDC_COMBO1, &CDlg_ATxmegaBCD::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO2, &CDlg_ATxmegaBCD::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO3, &CDlg_ATxmegaBCD::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO4, &CDlg_ATxmegaBCD::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO5, &CDlg_ATxmegaBCD::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO6, &CDlg_ATxmegaBCD::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO7, &CDlg_ATxmegaBCD::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO8, &CDlg_ATxmegaBCD::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO9, &CDlg_ATxmegaBCD::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO10, &CDlg_ATxmegaBCD::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO11, &CDlg_ATxmegaBCD::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO12, &CDlg_ATxmegaBCD::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO13, &CDlg_ATxmegaBCD::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO14, &CDlg_ATxmegaBCD::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO15, &CDlg_ATxmegaBCD::OnCbnSelchangeCombo)
	ON_BN_CLICKED(IDC_CHECK1, &CDlg_ATxmegaBCD::OnBnClickedEnable)
END_MESSAGE_MAP()


// Dlg_ATxmegaBCD message handlers
BOOL CDlg_ATxmegaBCD::OnInitDialog()
{
...
BOOL CDlg_ATxmegaBCD::OnSetActive()
{
	if (*m_pDataLen != NULL)
	{
		...
		memcpy((void*)&Lockbits, m_RegBuff + 4, sizeof(Lockbits));
...
void CDlg_ATxmegaBCD::OnCbnSelchangeCombo()
{
	CString Cfg1;
	CString Cfg2;
	CString Cfg4;
...
void CDlg_ATxmegaBCD::OnOK()
{
	...
	if (state == 0x01)
	{
		...
		memcpy(m_RegBuff + sizeof(Fuse1)+sizeof(Fuse2)+sizeof(Fuse4)+sizeof(Fuse5), &Lockbits, sizeof(Lockbits));

		unsigned short int restdata1 = 0xFFFF;
		unsigned char restdata2 = 0xFF;
		memcpy(m_RegBuff + sizeof(Fuse1)+sizeof(Fuse2)+sizeof(Fuse4)+sizeof(Fuse5)+sizeof(Lockbits), &restdata1, sizeof(restdata1));
		memcpy(m_RegBuff + sizeof(Fuse1)+sizeof(Fuse2)+sizeof(Fuse4)+sizeof(Fuse5)+sizeof(Lockbits)+sizeof(restdata1), &restdata2, sizeof(restdata2));
		*m_pDataLen = sizeof(Fuse1)+sizeof(Fuse2)+sizeof(Fuse4)+sizeof(Fuse5)+sizeof(Lockbits)+sizeof(restdata1)+sizeof(restdata2);
	}
	else if (state == 0x0)
		*m_pDataLen = 0;
...
LRESULT CDlg_ATxmegaBCD::OnWizardNext()
{
	OnOK();
	return CMFCPropertyPage::OnWizardNext();

}
LRESULT CDlg_ATxmegaBCD::OnWizardBack()
{
	OnOK();
	return CMFCPropertyPage::OnWizardBack();
}

void CDlg_ATxmegaBCD::OnBnClickedEnable()
{
	CButton* Encheck = (CButton*)GetDlgItem(IDC_CHECK1);
	int state = Encheck->GetCheck();
	if (state == 0x1)
