// Dlg_ATxmegaE.cpp : implementation file
//

#include "stdafx.h"
#include "Dlg_ATxmegaE.h"
...
IMPLEMENT_DYNAMIC(CDlg_ATxmegaE, CMFCPropertyPage)

CDlg_ATxmegaE::CDlg_ATxmegaE(struct chip_info_c *ChipInfo, unsigned char *RegisterBuff, unsigned long BuffLen, unsigned long *pDataLen, CWnd* pParent /*=NULL*/)
: CMFCPropertyPage(CDlg_ATxmegaE::IDD),
m_ChipInfo(ChipInfo),
...
CDlg_ATxmegaE::~CDlg_ATxmegaE()
{
}

void CDlg_ATxmegaE::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO4, m_BODPD);
	DDX_Control(pDX, IDC_COMBO5, m_RSTDISBL);
	DDX_Control(pDX, IDC_COMBO6, m_Startuptime);
	DDX_Control(pDX, IDC_COMBO7, m_WDLOCK);
	DDX_Control(pDX, IDC_COMBO8, m_BODACT);
	DDX_Control(pDX, IDC_COMBO9, m_EESAVE);
	DDX_Control(pDX, IDC_COMBO10, m_BODLEVEL);
	DDX_Control(pDX, IDC_COMBO11, m_FDACT5);
	DDX_Control(pDX, IDC_COMBO12, m_FDACT4);
	...
	DDX_Control(pDX, IDC_EDIT4, m_Cfg4);
	DDX_Control(pDX, IDC_EDIT5, m_Cfg5);
	DDX_Control(pDX, IDC_EDIT6, m_Cfg6);
	DDX_Control(pDX, IDC_EDIT7, m_Cfg7);
}


BEGIN_MESSAGE_MAP(CDlg_ATxmegaE, CMFCPropertyPage)
	ON_CBN_SELCHANGE(IDC_COMBO1, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO2, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO3, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO4, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO5, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO6, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO7, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO8, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO9, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO10, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO11, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO12, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO13, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO14, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO15, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO16, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_EDIT1, &CDlg_ATxmegaE::OnCbnSelchangeCombo)
	ON_EN_CHANGE(IDC_EDIT1, &CDlg_ATxmegaE::OnEnChangePortValue)
	ON_BN_CLICKED(IDC_CHECK1, &CDlg_ATxmegaE::OnBnClickedEnable)
END_MESSAGE_MAP()


// Dlg_ATxmegaE message handlers
BOOL CDlg_ATxmegaE::OnInitDialog()
{
	...
	m_FDACT5.InsertString(m_FDACT5.GetCount(), L"0 = Set by Port Pin n Value");
	m_FDACT5.InsertString(m_FDACT5.GetCount(), L"1 = Default Value");

	m_FDACT4.InsertString(m_FDACT4.GetCount(), L"0 = Set by Port Pin n Value");
	m_FDACT4.InsertString(m_FDACT4.GetCount(), L"1 = Default Value");
	...
	m_FDACT5.SetDroppedWidth(300);
	m_FDACT4.SetDroppedWidth(300);
	...
	m_Cfg5.SetWindowTextW(L"FF");
	m_Cfg5.EnableWindow(FALSE);
	m_Cfg6.SetWindowTextW(L"FF");
	m_Cfg6.EnableWindow(FALSE);
	m_Cfg7.SetWindowTextW(L"FF");
	m_Cfg7.EnableWindow(FALSE);
	...
	m_FDACT5.SetCurSel(1);
	m_FDACT4.SetCurSel(1);
	m_BLBB.SetCurSel(3);
	m_BLBA.SetCurSel(3);
	m_BLBAT.SetCurSel(3);
	m_LB.SetCurSel(2);
	...
	m_FDACT5.EnableWindow(FALSE);
	m_FDACT4.EnableWindow(FALSE);
...
BOOL CDlg_ATxmegaE::OnSetActive()
{
	if (*m_pDataLen != NULL)
	{
		...
		m_FDACT5.EnableWindow(TRUE);
		m_FDACT4.EnableWindow(TRUE);
		...
		memcpy((void*)&Fuse1, m_RegBuff, sizeof(Fuse1));
		Cfg1.Format(_T("%02X"), Fuse1);
		m_Cfg1.SetWindowTextW(Cfg1);

		memcpy((void*)&Fuse2, m_RegBuff + 1, sizeof(Fuse2));
		Cfg2.Format(_T("%02X"), Fuse2);
		m_Cfg2.SetWindowTextW(Cfg2);

		memcpy((void*)&Fuse4, m_RegBuff + 2, sizeof(Fuse4));
		Cfg4.Format(_T("%02X"), Fuse4);
		m_Cfg4.SetWindowTextW(Cfg4);

		memcpy((void*)&Fuse5, m_RegBuff + 3, sizeof(Fuse5));
		Cfg5.Format(_T("%02X"), Fuse5);
		m_Cfg5.SetWindowTextW(Cfg5);

		memcpy((void*)&Fuse6, m_RegBuff + 4, sizeof(Fuse6));
		Cfg6.Format(_T("%02X"), Fuse6);
		m_Cfg6.SetWindowTextW(Cfg6);
		value_temp = Fuse6 & 0x3F;
		value_str.Format(_T("%02X"), value_temp);
		m_VALUE.SetWindowTextW(value_str);


		memcpy((void*)&Lockbits, m_RegBuff + 5, sizeof(Lockbits));
		Cfg7.Format(_T("%02X"), Lockbits);
		m_Cfg7.SetWindowTextW(Cfg7);
		...
		//Fault Detection Action Timer/Counter5
		if ((Fuse6 & 0x80) == 0x80)
			m_FDACT5.SetCurSel(1);
		else if ((Fuse6 & 0x80) == 0x00)
			m_FDACT5.SetCurSel(0);

		//Fault Detection Action Timer/Counter4
		if ((Fuse6 & 0x40) == 0x40)
			m_FDACT4.SetCurSel(1);
		else if ((Fuse6 & 0x40) == 0x00)
			m_FDACT4.SetCurSel(0);
...
void CDlg_ATxmegaE::OnCbnSelchangeCombo()
{
	CString Cfg1;
	CString Cfg2;
	CString Cfg4;
	...
	//Fault Detection Action Timer/Counter5
	Fuse6 &= 0x7F;
	if (m_FDACT5.GetCurSel() == 1)
		Fuse6 |= 0x80;
	else if (m_FDACT5.GetCurSel() == 0)
		Fuse6 |= 0x00;
	Cfg6.Format(_T("%02X"), Fuse6);
	m_Cfg6.SetWindowTextW(Cfg6);
	...
	//Fault Detection Action Timer/Counter4
	Fuse6 &= 0xBF;
	if (m_FDACT4.GetCurSel() == 1)
		Fuse6 |= 0x40;
	else if (m_FDACT4.GetCurSel() == 0)
		Fuse6 |= 0x00;
	Cfg6.Format(_T("%02X"), Fuse6);
	m_Cfg6.SetWindowTextW(Cfg6);
	...
	//Port Pin n Value
	Fuse6 &= 0xC0;
	m_VALUE.GetWindowTextW(value_Str);
	value_temp = numeric_conversion::hexstring_to_size_t(value_Str.GetString());
	value_temp &= 0x3F;
	Fuse6 |= value_temp;
	Cfg6.Format(_T("%02X"), Fuse6);
	m_Cfg6.SetWindowTextW(Cfg6);
...
void CDlg_ATxmegaE::OnOK()
{
	...
	if (state == 0x01)
	{
		...
		memcpy(m_RegBuff, &Fuse1, sizeof(Fuse1));

		m_Cfg2.GetWindowTextW(temp);
		Fuse2 = numeric_conversion::hexstring_to_size_t(temp.GetString());
		memcpy(m_RegBuff + sizeof(Fuse1), &Fuse2, sizeof(Fuse2));

		m_Cfg4.GetWindowTextW(temp);
		Fuse4 = numeric_conversion::hexstring_to_size_t(temp.GetString());
		memcpy(m_RegBuff + sizeof(Fuse1)+sizeof(Fuse2), &Fuse4, sizeof(Fuse4));

		m_Cfg5.GetWindowTextW(temp);
		Fuse5 = numeric_conversion::hexstring_to_size_t(temp.GetString());
		memcpy(m_RegBuff + sizeof(Fuse1)+sizeof(Fuse2)+sizeof(Fuse4), &Fuse5, sizeof(Fuse5));

		m_Cfg6.GetWindowTextW(temp);
		Fuse6 = numeric_conversion::hexstring_to_size_t(temp.GetString());
		memcpy(m_RegBuff + sizeof(Fuse1)+sizeof(Fuse2)+sizeof(Fuse4)+sizeof(Fuse5), &Fuse6, sizeof(Fuse6));

		m_Cfg7.GetWindowTextW(temp);
		Lockbits = numeric_conversion::hexstring_to_size_t(temp.GetString());
		memcpy(m_RegBuff + sizeof(Fuse1)+sizeof(Fuse2)+sizeof(Fuse4)+sizeof(Fuse5)+sizeof(Fuse6), &Lockbits, sizeof(Lockbits));

		unsigned short int restdata = 0xFFFF;
		memcpy(m_RegBuff + sizeof(Fuse1)+sizeof(Fuse2)+sizeof(Fuse4)+sizeof(Fuse5)+sizeof(Fuse6)+sizeof(Lockbits), &restdata, sizeof(restdata));
		*m_pDataLen = sizeof(Fuse1)+sizeof(Fuse2)+sizeof(Fuse4)+sizeof(Fuse5)+sizeof(Fuse6)+sizeof(Lockbits)+sizeof(restdata);
	}
	else if (state == 0x0)
		*m_pDataLen = 0;
...
LRESULT CDlg_ATxmegaE::OnWizardNext()
{
	OnOK();
	return CMFCPropertyPage::OnWizardNext();

}
LRESULT CDlg_ATxmegaE::OnWizardBack()
{
	OnOK();
	return CMFCPropertyPage::OnWizardBack();
}

void CDlg_ATxmegaE::OnBnClickedEnable()
{
	CButton* Encheck = (CButton*)GetDlgItem(IDC_CHECK1);
	int state = Encheck->GetCheck();
	if (state == 0x1)
...
void CDlg_ATxmegaE::OnEnChangePortValue()
{
	...
	value_temp &= 0x3F;
	Fuse6 |= value_temp;
	fuse6_temp.Format(_T("%02X"), Fuse6);
	m_Cfg6.SetWindowTextW(fuse6_temp);
