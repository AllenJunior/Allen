// Dlg_AT90CAN.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CDlg_AT90CAN, CMFCPropertyPage)

CDlg_AT90CAN::CDlg_AT90CAN(struct chip_info_c *ChipInfo, unsigned char *RegisterBuff, unsigned long BuffLen, unsigned long *pDataLen, CWnd* pParent /*=NULL*/)
: CMFCPropertyPage(CDlg_AT90CAN::IDD),
m_ChipInfo(ChipInfo),
...
CDlg_AT90CAN::~CDlg_AT90CAN()
{
}

void CDlg_AT90CAN::DoDataExchange(CDataExchange* pDX)
{
	CMFCPropertyPage::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_COMBO1, m_BODLevel);
	DDX_Control(pDX, IDC_COMBO2, m_TA0SEL);
...
BEGIN_MESSAGE_MAP(CDlg_AT90CAN, CMFCPropertyPage)
	ON_CBN_SELCHANGE(IDC_COMBO1, &CDlg_AT90CAN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO2, &CDlg_AT90CAN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO3, &CDlg_AT90CAN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO4, &CDlg_AT90CAN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO5, &CDlg_AT90CAN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO6, &CDlg_AT90CAN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO7, &CDlg_AT90CAN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO8, &CDlg_AT90CAN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO9, &CDlg_AT90CAN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO10, &CDlg_AT90CAN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO11, &CDlg_AT90CAN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO12, &CDlg_AT90CAN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO13, &CDlg_AT90CAN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO14, &CDlg_AT90CAN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO15, &CDlg_AT90CAN::OnCbnSelchangeCombo)
	ON_BN_CLICKED(IDC_CHECK1, &CDlg_AT90CAN::OnBnClickedEnable)
END_MESSAGE_MAP()


// Dlg_AT90CAN message handlers
BOOL CDlg_AT90CAN::OnInitDialog()
{
	...
	//Extended Fuse
	m_BODLevel.InsertString(m_BODLevel.GetCount(), L"000 = Trigger Level at 2.5V");
	m_BODLevel.InsertString(m_BODLevel.GetCount(), L"001 = Trigger Level at 2.6V");
	m_BODLevel.InsertString(m_BODLevel.GetCount(), L"010 = Trigger Level at 2.7V");
	m_BODLevel.InsertString(m_BODLevel.GetCount(), L"011 = Trigger Level at 3.8V");
	m_BODLevel.InsertString(m_BODLevel.GetCount(), L"100 = Trigger Level at 3.9V");
	m_BODLevel.InsertString(m_BODLevel.GetCount(), L"101 = Trigger Level at 4.0V");
	m_BODLevel.InsertString(m_BODLevel.GetCount(), L"110 = Trigger Level at 4.1V");
	m_BODLevel.InsertString(m_BODLevel.GetCount(), L"111 = BOD Disable");//initial value
	m_TA0SEL.InsertString(m_TA0SEL.GetCount(), L"0 = Enable");
	m_TA0SEL.InsertString(m_TA0SEL.GetCount(), L"1 = Disable");
	//High Fuse
...
void CDlg_AT90CAN::OnCbnSelchangeCombo()
{
	...
	//TA0SEL
	ExtendedFuse &= 0xFE;
	if (m_TA0SEL.GetCurSel() == 1)
		ExtendedFuse |= 0x01;
	else if (m_TA0SEL.GetCurSel() == 0)
		ExtendedFuse |= 0x00;
	ExFuse_str.Format(_T("%02X"), ExtendedFuse);
	m_ExFuse.SetWindowTextW(ExFuse_str);
	//OCDEN
...
LRESULT CDlg_AT90CAN::OnWizardNext()
{
	OnOK();
	return CMFCPropertyPage::OnWizardNext();

}
LRESULT CDlg_AT90CAN::OnWizardBack()
{
	OnOK();
	return CMFCPropertyPage::OnWizardBack();
}

void CDlg_AT90CAN::OnBnClickedEnable()
{
	CButton* Encheck = (CButton*)GetDlgItem(IDC_CHECK1);
	int state = Encheck->GetCheck();
	if (state == 1)
