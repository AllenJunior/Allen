// Dlg_AT90PWMX.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CDlg_AT90PWMX, CMFCPropertyPage)

CDlg_AT90PWMX::CDlg_AT90PWMX(struct chip_info_c *ChipInfo, unsigned char *RegisterBuff, unsigned long BuffLen, unsigned long *pDataLen, CWnd* pParent /*=NULL*/)
: CMFCPropertyPage(CDlg_AT90PWMX::IDD),
m_ChipInfo(ChipInfo),
...
CDlg_AT90PWMX::~CDlg_AT90PWMX()
{
}

void CDlg_AT90PWMX::DoDataExchange(CDataExchange* pDX)
{
	CPropertyPage::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_CHECK1, Option_enable);
	DDX_Control(pDX, IDC_COMBO1, m_PSC2RB);
	DDX_Control(pDX, IDC_COMBO2, m_PSC1RB);
	DDX_Control(pDX, IDC_COMBO3, m_PSC0RB);
	DDX_Control(pDX, IDC_COMBO4, m_PSCRV);
	DDX_Control(pDX, IDC_COMBO5, m_BOOTSZ);
	DDX_Control(pDX, IDC_COMBO6, m_BOOTRST);
	...
	DDX_Control(pDX, IDC_COMBO13, m_CKDIV8);
	DDX_Control(pDX, IDC_COMBO14, m_CKOUT);
	DDX_Control(pDX, IDC_COMBO15, m_CKSEL);
	DDX_Control(pDX, IDC_COMBO16, m_BLBB);
	DDX_Control(pDX, IDC_COMBO17, m_BLBA);
	DDX_Control(pDX, IDC_COMBO18, m_LOCK);
...
BEGIN_MESSAGE_MAP(CDlg_AT90PWMX, CMFCPropertyPage)
	ON_CBN_SELCHANGE(IDC_COMBO1, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO2, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO3, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO4, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO5, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO6, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO7, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO8, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO9, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO10, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO11, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO12, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO13, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO14, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO15, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO16, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO17, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO18, &CDlg_AT90PWMX::OnCbnSelchangeCombo)
	ON_BN_CLICKED(IDC_CHECK1, &CDlg_AT90PWMX::OnBnClickedEnable)
END_MESSAGE_MAP()


// Dlg_AT90PWMX message handlers
BOOL CDlg_AT90PWMX::OnInitDialog()
{
	...
	LowFuse = 0x62;
	Lockbits = 0xFF;
	m_ExFuse.SetWindowTextW(L"F9");
	m_HFuse.SetWindowTextW(L"DF");
	m_LFuse.SetWindowTextW(L"62");
	m_Lockbits.SetWindowTextW(L"FF");
	//Extended Fuse
	m_PSC2RB.InsertString(m_PSC2RB.GetCount(), L"0 = PSC2RB RST Enable");
	m_PSC2RB.InsertString(m_PSC2RB.GetCount(), L"1 = PSC2RB RST Disable");//default
	m_PSC1RB.InsertString(m_PSC1RB.GetCount(), L"0 = PSC1RB RST Enable");
	m_PSC1RB.InsertString(m_PSC1RB.GetCount(), L"1 = PSC1RB RST Disable");//default
	m_PSC0RB.InsertString(m_PSC0RB.GetCount(), L"0 = PSC0RB RST Enable");
	m_PSC0RB.InsertString(m_PSC0RB.GetCount(), L"1 = PSC0RB RST Disable");//default
	m_PSCRV.InsertString(m_PSCRV.GetCount(), L"0 = PSCOUT RST Value");
	m_PSCRV.InsertString(m_PSCRV.GetCount(), L"1 = PSCOUT RST Value");//default
	...
	m_BODLevel.InsertString(m_BODLevel.GetCount(), L"100 = Trigger Level at 4.5V");
	...
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,0010 = Int.RC OSC:8MHz,StartupTime:6CK+65ms");  	// 5 = 0x22
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,0011 = PLL CLK 4:16,StartupTime:1KCK"); 				// 6 = 0x03
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,0011 = PLL CLK 4:16,StartupTime:1KCK+4ms"); 	// 7 = 0x13
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,0011 = PLL CLK 4:16,StartupTime:1KCK+64ms");  	// 8 = 0x23
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,0011 = PLL CLK 4:16,StartupTime:16KCK");  			// 9 = 0x33
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1000 = Ext.Crystal OSC:0.4-0.9MHz,StartupTime:258CK+4.1ms"); // 10 = 0x08
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1000 = Ext.Crystal OSC:0.4-0.9MHz,StartupTime:258CK+65ms"); 	// 11 = 0x18
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1000 = Ext.Crystal OSC:0.4-0.9MHz,StartupTime:1KCK"); 		// 12 = 0x28
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1000 = Ext.Crystal OSC:0.4-0.9MHz,StartupTime:1KCK+4.1ms");  // 13 = 0x38
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1001 = Ext.Crystal OSC:0.4-0.9MHz,StartupTime:1KCK+65ms"); 	// 14 = 0x09
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1001 = Ext.Crystal OSC:0.4-0.9MHz,StartupTime:16KCK"); 		// 15 = 0x19
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1001 = Ext.Crystal OSC:0.4-0.9MHz,StartupTime:16KCK+4.1ms"); // 16 = 0x29
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1001 = Ext.Crystal OSC:0.4-0.9MHz,StartupTime:16KCK+65ms");  // 17 = 0x39
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1010 = Ext.Crystal OSC:0.9-3.0MHz,StartupTime:258CK+4.1ms"); // 18 = 0x0A
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1010 = Ext.Crystal OSC:0.9-3.0MHz,StartupTime:258CK+65ms");  // 19 = 0x1A
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1010 = Ext.Crystal OSC:0.9-3.0MHz,StartupTime:1KCK");		   // 20 = 0x2A
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1010 = Ext.Crystal OSC:0.9-3.0MHz,StartupTime:1KCK+4.1ms");  // 21 = 0x3A
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1011 = Ext.Crystal OSC:0.9-3.0MHz,StartupTime:1KCK+65ms");	// 22 = 0x0B
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1011 = Ext.Crystal OSC:0.9-3.0MHz,StartupTime:16KCK"); 	 	// 23 = 0x1B
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1011 = Ext.Crystal OSC:0.9-3.0MHz,StartupTime:16KCK+4.1ms");	// 24 = 0x2B
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1011 = Ext.Crystal OSC:0.9-3.0MHz,StartupTime:16KCK+65ms");   // 25 = 0x3B
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1100 = Ext.Crystal OSC:3-8MHz,StartupTime:258CK+4.1ms");	 // 26 = 0x0C
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1100 = Ext.Crystal OSC:3-8MHz,StartupTime:258CK+65ms"); 	 // 27 = 0x1C
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1100 = Ext.Crystal OSC:3-8MHz,StartupTime:1KCK");	 		 // 28 = 0x2C
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1100 = Ext.Crystal OSC:3-8MHz,StartupTime:1KCK+4.1ms");    // 29 = 0x3C
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1101 = Ext.Crystal OSC:3-8MHz,StartupTime:1KCK+65ms");	 	// 30 = 0x0D
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1101 = Ext.Crystal OSC:3-8MHz,StartupTime:16KCK"); 	 	 	// 31 = 0x1D
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1101 = Ext.Crystal OSC:3-8MHz,StartupTime:16KCK+4.1ms");	 	// 32 = 0x2D
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1101 = Ext.Crystal OSC:3-8MHz,StartupTime:16KCK+65ms");   	// 33 = 0x3D
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1110 = Ext.Crystal OSC:>8MHz,StartupTime:258CK+4.1ms");		// 34 = 0x0E
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1110 = Ext.Crystal OSC:>8MHz,StartupTime:258CK+65ms"); 	 	// 35 = 0x1E
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1110 = Ext.Crystal OSC:>8MHz,StartupTime:1KCK");	 			// 36 = 0x2E
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1110 = Ext.Crystal OSC:>8MHz,StartupTime:1KCK+4.1ms");   		// 37 = 0x3E
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1111 = Ext.Crystal OSC:>8MHz,StartupTime:1KCK+65ms");			// 38 = 0x0F
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1111 = Ext.Crystal OSC:>8MHz,StartupTime:16KCK"); 	 		// 39 = 0x1F
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1111 = Ext.Crystal OSC:>8MHz,StartupTime:16KCK+4.1ms");	 	// 40 = 0x2F
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1111 = Ext.Crystal OSC:>8MHz,StartupTime:16KCK+65ms");   		// 41 = 0x3F
	//LOCK
	...
	m_PSC1RB.SetCurSel(1);
	m_PSC0RB.SetCurSel(1);
	m_PSCRV.SetCurSel(1);
	...
	m_PSC1RB.EnableWindow(FALSE);
	m_PSC0RB.EnableWindow(FALSE);
...
BOOL CDlg_AT90PWMX::OnSetActive()
{
	if (*m_pDataLen != NULL)
	{
		CButton* Encheck = (CButton*)GetDlgItem(IDC_CHECK1);
		...
		m_PSC1RB.EnableWindow(TRUE);
		m_PSC0RB.EnableWindow(TRUE);
...
void CDlg_AT90PWMX::OnCbnSelchangeCombo()
{
	...
	//PSC1RB
	ExtendedFuse &= 0xBF;
	if (m_PSC1RB.GetCurSel() == 1)
		ExtendedFuse |= 0x40;
	else if (m_PSC1RB.GetCurSel() == 0)
		ExtendedFuse |= 0x00;
	ExFuse_str.Format(_T("%02X"), ExtendedFuse);
	m_ExFuse.SetWindowTextW(ExFuse_str);
	//PSC0RB
	ExtendedFuse &= 0xDF;
	if (m_PSC0RB.GetCurSel() == 1)
		ExtendedFuse |= 0x20;
	else if (m_PSC0RB.GetCurSel() == 0)
		ExtendedFuse |= 0x00;
	ExFuse_str.Format(_T("%02X"), ExtendedFuse);
	m_ExFuse.SetWindowTextW(ExFuse_str);
	//PSCRV
...
LRESULT CDlg_AT90PWMX::OnWizardNext()
{
	OnOK();
	return CMFCPropertyPage::OnWizardNext();

}
LRESULT CDlg_AT90PWMX::OnWizardBack()
{
	OnOK();
	return CMFCPropertyPage::OnWizardBack();
}

void CDlg_AT90PWMX::OnBnClickedEnable()
{
	CButton* Encheck = (CButton*)GetDlgItem(IDC_CHECK1);
	int state = Encheck->GetCheck();
	if (state == 1)
