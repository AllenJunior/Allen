// Dlg_AT90PWMX1.cpp : implementation file
//
...
// Dlg_AT90PWMX1 dialog

IMPLEMENT_DYNAMIC(CDlg_AT90PWMX1, CMFCPropertyPage)

CDlg_AT90PWMX1::CDlg_AT90PWMX1(struct chip_info_c *ChipInfo, unsigned char *RegisterBuff, unsigned long BuffLen, unsigned long *pDataLen, CWnd* pParent /*=NULL*/)
: CMFCPropertyPage(CDlg_AT90PWMX1::IDD),
m_ChipInfo(ChipInfo),
...
CDlg_AT90PWMX1::~CDlg_AT90PWMX1()
{
}

void CDlg_AT90PWMX1::DoDataExchange(CDataExchange* pDX)
{
	CPropertyPage::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_CHECK1, Option_enable);
	DDX_Control(pDX, IDC_COMBO1, m_PSC2RB);
	DDX_Control(pDX, IDC_COMBO2, m_PSC2RBA);
	DDX_Control(pDX, IDC_COMBO3, m_PSCRRB);
	DDX_Control(pDX, IDC_COMBO4, m_PSCRV);
	DDX_Control(pDX, IDC_COMBO5, m_PSCINRB);
	DDX_Control(pDX, IDC_COMBO6, m_BODLevel);
	DDX_Control(pDX, IDC_COMBO7, m_RSTDISBL);
	DDX_Control(pDX, IDC_COMBO8, m_DWEN);
	DDX_Control(pDX, IDC_COMBO9, m_SPIEN);
	DDX_Control(pDX, IDC_COMBO10, m_WDTON);
	DDX_Control(pDX, IDC_COMBO11, m_EESAVE);
	DDX_Control(pDX, IDC_COMBO12, m_BOOTSZ);
	DDX_Control(pDX, IDC_COMBO13, m_BOOTRST);
	DDX_Control(pDX, IDC_COMBO14, m_CKDIV8);
	DDX_Control(pDX, IDC_COMBO15, m_CKOUT);
	DDX_Control(pDX, IDC_COMBO16, m_CKSEL);
	DDX_Control(pDX, IDC_COMBO17, m_BLBB);
	DDX_Control(pDX, IDC_COMBO18, m_BLBA);
	DDX_Control(pDX, IDC_COMBO19, m_LOCK);
...
BEGIN_MESSAGE_MAP(CDlg_AT90PWMX1, CMFCPropertyPage)
	ON_CBN_SELCHANGE(IDC_COMBO1, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO2, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO3, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO4, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO5, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO6, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO7, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO8, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO9, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO10, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO11, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO12, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO13, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO14, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO15, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO16, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO17, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO18, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO19, &CDlg_AT90PWMX1::OnCbnSelchangeCombo)
	ON_BN_CLICKED(IDC_CHECK1, &CDlg_AT90PWMX1::OnBnClickedEnable)
END_MESSAGE_MAP()


// Dlg_AT90PWMX1 message handlers
BOOL CDlg_AT90PWMX1::OnInitDialog()
{
	...
	//Extended Fuse
	m_PSC2RB.InsertString(m_PSC2RB.GetCount(), L"0 = RST Behavior Enable");
	m_PSC2RB.InsertString(m_PSC2RB.GetCount(), L"1 = RST Behavior Disable");//initial value
	m_PSC2RBA.InsertString(m_PSC2RBA.GetCount(), L"0 = RST Behavior Enable");
	m_PSC2RBA.InsertString(m_PSC2RBA.GetCount(), L"1 = RST Behavior Disable");//initial value
	m_PSCRRB.InsertString(m_PSCRRB.GetCount(), L"0 = Reduced RST Behavior Enable");
	m_PSCRRB.InsertString(m_PSCRRB.GetCount(), L"1 = Reduced RST Behavior Disable");//initial value
	m_PSCRV.InsertString(m_PSCRV.GetCount(), L"0 = PSC & PSCR RST Value");
	m_PSCRV.InsertString(m_PSCRV.GetCount(), L"1 = PSC & PSCR RST Value");//initial value
	m_PSCINRB.InsertString(m_PSCINRB.GetCount(), L"0 = PSC & PSCR RST Behavior Enable");
	m_PSCINRB.InsertString(m_PSCINRB.GetCount(), L"1 = PSC & PSCR RST Behavior Disable");//initial value
	...
	m_BODLevel.InsertString(m_BODLevel.GetCount(), L"011 = Trigger Level at 4.4V");
	...
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,0110 = Int.RC OSC:1MHz,StartupTime:6CK/14CK(PWRDWN/RST)"); 		// 21 = 0x06
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,0110 = Int.RC OSC:1MHz,StartupTime:6CK/14CK+4.1ms(PWRDWN/RST)"); 	// 22 = 0x16
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,0110 = Int.RC OSC:1MHz,StartupTime:6CK/14CK+65ms(PWRDWN/RST)");  	// 23 = 0xx26
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1000 = Ext.Crystal.OSC:0.4-0.9MHz,StartupTime:258CK/14CK+4.1ms(PWRDWN/RST)"); // 24 = 0x08
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1000 = Ext.Crystal.OSC:0.4-0.9MHz,StartupTime:258CK/14CK+65ms(PWRDWN/RST)");  // 25 = 0x18
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1000 = Ext.Crystal.OSC:0.4-0.9MHz,StartupTime:1KCK/14CK(PWRDWN/RST)"); 		// 26 = 0x28
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1000 = Ext.Crystal.OSC:0.4-0.9MHz,StartupTime:1KCK/14CK+4.1ms(PWRDWN/RST)");  // 27 = 0x38
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1001 = Ext.Crystal.OSC:0.4-0.9MHz,StartupTime:1KCK/14CK+65ms(PWRDWN/RST)"); 	// 28 = 0x09
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1001 = Ext.Crystal.OSC:0.4-0.9MHz,StartupTime:16KCK/14CK(PWRDWN/RST)"); 		// 29 = 0x19
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1001 = Ext.Crystal.OSC:0.4-0.9MHz,StartupTime:16KCK/14CK+4.1ms(PWRDWN/RST)"); // 30 = 0x29
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1001 = Ext.Crystal.OSC:0.4-0.9MHz,StartupTime:16KCK/14CK+65ms(PWRDWN/RST)");  // 31 = 0x39
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1010 = Ext.Crystal.OSC:0.9-3.0MHz,StartupTime:258CK/14CK+4.1ms(PWRDWN/RST)"); // 32 = 0x0A
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1010 = Ext.Crystal.OSC:0.9-3.0MHz,StartupTime:258CK/14CK+65ms(PWRDWN/RST)"); 	// 33 = 0x1A
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1010 = Ext.Crystal.OSC:0.9-3.0MHz,StartupTime:1KCK/14CK(PWRDWN/RST)");		// 34 = 0x2A
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1010 = Ext.Crystal.OSC:0.9-3.0MHz,StartupTime:1KCK/14CK+4.1ms(PWRDWN/RST)");  // 35 = 0x3A
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1011 = Ext.Crystal.OSC:0.9-3.0MHz,StartupTime:1KCK/14CK+65ms(PWRDWN/RST)");	// 36 = 0x0B
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1011 = Ext.Crystal.OSC:0.9-3.0MHz,StartupTime:16KCK/14CK(PWRDWN/RST)"); 	 	// 37 = 0x1B
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1011 = Ext.Crystal.OSC:0.9-3.0MHz,StartupTime:16KCK/14CK+4.1ms(PWRDWN/RST)");	// 38 = 0x2B
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1011 = Ext.Crystal.OSC:0.9-3.0MHz,StartupTime:16KCK/14CK+65ms(PWRDWN/RST)");  // 39 = 0x3B
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1100 = Ext.Crystal.OSC:3-8MHz,StartupTime:258CK/14CK+4.1ms((PWRDWN/RST)");	// 40 = 0x0C
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1100 = Ext.Crystal.OSC:3-8MHz,StartupTime:258CK/14CK+65ms(PWRDWN/RST)"); 	 	// 41 = 0x1C
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1100 = Ext.Crystal.OSC:3-8MHz,StartupTime:1KCK/14CK(PWRDWN/RST)");	 		// 42 = 0x2C
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1100 = Ext.Crystal.OSC:3-8MHz,StartupTime:1KCK/14CK+4.1ms(PWRDWN/RST)");   	// 43 = 0x3C
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1101 = Ext.Crystal.OSC:3-8MHz,StartupTime:1KCK/14CK+65ms(PWRDWN/RST)");	 	// 44 = 0x0D
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1101 = Ext.Crystal.OSC:3-8MHz,StartupTime:16KCK/14CK(PWRDWN/RST)"); 	 	 	// 45 = 0x1D
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1101 = Ext.Crystal.OSC:3-8MHz,StartupTime:16KCK/14CK+4.1ms(PWRDWN/RST)");	 	// 46 = 0x2D
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1101 = Ext.Crystal.OSC:3-8MHz,StartupTime:16KCK/14CK+65ms(PWRDWN/RST)");   	// 47 = 0x3D
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1110 = Ext.Crystal.OSC:>8MHz,StartupTime:258CK/14CK+4.1ms(PWRDWN/RST)");		// 48 = 0x0E
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1110 = Ext.Crystal.OSC:>8MHz,StartupTime:258CK/14CK+65ms(PWRDWN/RST)"); 	 	// 49 = 0x1E
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1110 = Ext.Crystal.OSC:>8MHz,StartupTime:1KCK/14CK(PWRDWN/RST)");	 			// 50 = 0x2E
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1110 = Ext.Crystal.OSC:>8MHz,StartupTime:1KCK/14CK+4.1ms(PWRDWN/RST)");   	// 51 = 0x3E
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1111 = Ext.Crystal.OSC:>8MHz,StartupTime:1KCK/14CK+65ms(PWRDWN/RST)");		// 52 = 0x0F
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1111 = Ext.Crystal.OSC:>8MHz,StartupTime:16KCK/14CK(PWRDWN/RST)"); 	 		// 53 = 0x1F
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1111 = Ext.Crystal.OSC:>8MHz,StartupTime:16KCK/14CK+4.1ms(PWRDWN/RST)");	 	// 54 = 0x2F
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1111 = Ext.Crystal.OSC:>8MHz,StartupTime:16KCK/14CK+65ms(PWRDWN/RST)");   	// 55 = 0x3F
	//LOCK
	...
	//SetCurSel
	//Extended Fuse
	m_PSC2RB.SetCurSel(1);
	m_PSC2RBA.SetCurSel(1);
	m_PSCRRB.SetCurSel(1);
	m_PSCRV.SetCurSel(1);
	m_PSCINRB.SetCurSel(1);
	...
	m_LOCK.SetCurSel(2);
	//Disable Combobox
	m_PSC2RB.EnableWindow(FALSE);
	m_PSC2RBA.EnableWindow(FALSE);
	m_PSCRRB.EnableWindow(FALSE);
	m_PSCRV.EnableWindow(FALSE);
	m_PSCINRB.EnableWindow(FALSE);
...
BOOL CDlg_AT90PWMX1::OnSetActive()
{
	if (*m_pDataLen != NULL)
	{
		CButton* Encheck = (CButton*)GetDlgItem(IDC_CHECK1);
		Encheck->SetCheck(1);
		m_PSC2RB.EnableWindow(TRUE);
		m_PSC2RBA.EnableWindow(TRUE);
		m_PSCRRB.EnableWindow(TRUE);
		m_PSCRV.EnableWindow(TRUE);
		m_PSCINRB.EnableWindow(TRUE);
		...
		//PSC2RB
		if ((ExtendedFuse & 0x80) == 0x80)
			m_PSC2RB.SetCurSel(1);
		else if ((ExtendedFuse & 0x80) == 0x00)
			m_PSC2RB.SetCurSel(0);
		//PSC2RBA
		if ((ExtendedFuse & 0x40) == 0x40)
			m_PSC2RBA.SetCurSel(1);
		else if ((ExtendedFuse & 0x40) == 0x00)
			m_PSC2RBA.SetCurSel(0);
		//PSCRRB
		if ((ExtendedFuse & 0x20) == 0x20)
			m_PSCRRB.SetCurSel(1);
		else if ((ExtendedFuse & 0x20) == 0x00)
			m_PSCRRB.SetCurSel(0);
		//PSCRV
...
void CDlg_AT90PWMX1::OnCbnSelchangeCombo()
{
	...
	//PSC2RB
	ExtendedFuse &= 0x7F;
	if (m_PSC2RB.GetCurSel() == 1)
		ExtendedFuse |= 0x80;
	else if (m_PSC2RB.GetCurSel() == 0)
		ExtendedFuse |= 0x00;
	ExFuse_str.Format(_T("%02X"), ExtendedFuse);
	m_ExFuse.SetWindowTextW(ExFuse_str);
	//PSC2RBA
	ExtendedFuse &= 0xBF;
	if (m_PSC2RBA.GetCurSel() == 1)
		ExtendedFuse |= 0x40;
	else if (m_PSC2RBA.GetCurSel() == 0)
		ExtendedFuse |= 0x00;
	ExFuse_str.Format(_T("%02X"), ExtendedFuse);
	m_ExFuse.SetWindowTextW(ExFuse_str);
	//PSCRRB
	ExtendedFuse &= 0xDF;
	if (m_PSCRRB.GetCurSel() == 1)
		ExtendedFuse |= 0x20;
	else if (m_PSCRRB.GetCurSel() == 0)
		ExtendedFuse |= 0x00;
	ExFuse_str.Format(_T("%02X"), ExtendedFuse);
	m_ExFuse.SetWindowTextW(ExFuse_str);
	//PSCRV
	ExtendedFuse &= 0xEF;
	if (m_PSCRV.GetCurSel() == 1)
		ExtendedFuse |= 0x10;
	else if (m_PSCRV.GetCurSel() == 0)
		ExtendedFuse |= 0x00;
	ExFuse_str.Format(_T("%02X"), ExtendedFuse);
	m_ExFuse.SetWindowTextW(ExFuse_str);
	//PSCINRB
	ExtendedFuse &= 0xF7;
	if (m_PSCINRB.GetCurSel() == 1)
		ExtendedFuse |= 0x08;
	else if (m_PSCINRB.GetCurSel() == 0)
		ExtendedFuse |= 0x00;
	ExFuse_str.Format(_T("%02X"), ExtendedFuse);
	m_ExFuse.SetWindowTextW(ExFuse_str);
	//BODLevel
...
LRESULT CDlg_AT90PWMX1::OnWizardNext()
{
	OnOK();
	return CMFCPropertyPage::OnWizardNext();

}
LRESULT CDlg_AT90PWMX1::OnWizardBack()
{
	OnOK();
	return CMFCPropertyPage::OnWizardBack();
}

void CDlg_AT90PWMX1::OnBnClickedEnable()
{
	CButton* Encheck = (CButton*)GetDlgItem(IDC_CHECK1);
	int state = Encheck->GetCheck();
	if (state == 1)
