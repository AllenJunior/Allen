// Dlg_AT90USB.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CDlg_AT90USB, CMFCPropertyPage)

CDlg_AT90USB::CDlg_AT90USB(struct chip_info_c *ChipInfo, unsigned char *RegisterBuff, unsigned long BuffLen, unsigned long *pDataLen, CWnd* pParent /*=NULL*/)
: CMFCPropertyPage(CDlg_AT90USB::IDD),
m_ChipInfo(ChipInfo),
...
CDlg_AT90USB::~CDlg_AT90USB()
{
}

void CDlg_AT90USB::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDlg_AT90USB, CMFCPropertyPage)
	ON_CBN_SELCHANGE(IDC_COMBO1, &CDlg_AT90USB::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO2, &CDlg_AT90USB::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO3, &CDlg_AT90USB::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO4, &CDlg_AT90USB::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO5, &CDlg_AT90USB::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO6, &CDlg_AT90USB::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO7, &CDlg_AT90USB::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO8, &CDlg_AT90USB::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO9, &CDlg_AT90USB::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO10, &CDlg_AT90USB::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO11, &CDlg_AT90USB::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO12, &CDlg_AT90USB::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO13, &CDlg_AT90USB::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO14, &CDlg_AT90USB::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO15, &CDlg_AT90USB::OnCbnSelchangeCombo)
	ON_BN_CLICKED(IDC_CHECK1, &CDlg_AT90USB::OnBnClickedEnable)

END_MESSAGE_MAP()


// Dlg_AT90USB message handlers
BOOL CDlg_AT90USB::OnInitDialog()
{
	CMFCPropertyPage::OnInitDialog();
	if (wcscmp(_T("AT90USBX2"), m_ChipInfo->description) == 0)
	{
		ExtendedFuse = 0xF4;
		HighFuse = 0xD9;
		LowFuse = 0x5E;
		...
		m_BODLevel.InsertString(m_BODLevel.GetCount(), L"010 = Trigger Level at 3.6V");
		...
		m_BODLevel.InsertString(m_BODLevel.GetCount(), L"101 = Trigger Level at 2.9V");
	...
	if (wcscmp(_T("AT90USB64X"), m_ChipInfo->description) == 0
		|| wcscmp(_T("AT90USB128X"), m_ChipInfo->description) == 0)
	{
		if (wcscmp(_T("AT90USB64X"), m_ChipInfo->description) == 0)
		{
			ExtendedFuse = 0xF3;
			...
			m_HFuse.SetWindowTextW(L"9B");
			m_LFuse.SetWindowTextW(L"62");
			m_Lockbits.SetWindowTextW(L"FF");
		}
		if (wcscmp(_T("AT90USB128X"), m_ChipInfo->description) == 0)
		{
			ExtendedFuse = 0xF3;
			HighFuse = 0x99;
			LowFuse = 0x62;
		...
		m_BODLevel.InsertString(m_BODLevel.GetCount(), L"011 = Trigger Level at 2.6V");
		m_BODLevel.InsertString(m_BODLevel.GetCount(), L"100 = Trigger Level at 2.4V");//initial value
	...
	m_BOOTSZ.InsertString(m_BOOTSZ.GetCount(), L"00 = Boot Size, please refer to datasheet");//initial value
	m_BOOTSZ.InsertString(m_BOOTSZ.GetCount(), L"01 = Boot Size, please refer to datasheet");
	m_BOOTSZ.InsertString(m_BOOTSZ.GetCount(), L"10 = Boot Size, please refer to datasheet");
	m_BOOTSZ.InsertString(m_BOOTSZ.GetCount(), L"11 = Boot Size, please refer to datasheet");
	...
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,0100 = Ext.Low.freq.crystal,StartupTime:1KCK"); 		// 10 = 0x04
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,0100 = Ext.Low.freq.crystal,StartupTime:1KCK+4.1ms"); 	// 11 = 0x14
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,0100 = Ext.Low.freq.crystal,StartupTime:1KCK+64ms");  	// 12 = 0x24
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,0101 = Ext.Low.freq.crystal,StartupTime:32KCK"); 		// 13 = 0x05
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,0101 = Ext.Low.freq.crystal,StartupTime:32KCK+4.1ms"); 	// 14 = 0x15
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,0101 = Ext.Low.freq.crystal,StartupTime:32KCK+64ms");  	// 15 = 0xx25
	...
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1100 = Ext.Crystal OSC:3-8MHz,StartupTime:258CK+4.1ms");	 // 32 = 0x0C
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1100 = Ext.Crystal OSC:3-8MHz,StartupTime:258CK+65ms"); 	 // 33 = 0x1C
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1100 = Ext.Crystal OSC:3-8MHz,StartupTime:1KCK");	 		 // 34 = 0x2C
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1100 = Ext.Crystal OSC:3-8MHz,StartupTime:1KCK+4.1ms");    // 35 = 0x3C
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1101 = Ext.Crystal OSC:3-8MHz,StartupTime:1KCK+65ms");	 	// 36 = 0x0D
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1101 = Ext.Crystal OSC:3-8MHz,StartupTime:16KCK"); 	 	 	// 37 = 0x1D
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1101 = Ext.Crystal OSC:3-8MHz,StartupTime:16KCK+4.1ms");	 	// 38 = 0x2D
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1101 = Ext.Crystal OSC:3-8MHz,StartupTime:16KCK+65ms");   	// 39 = 0x3D
...
void CDlg_AT90USB::OnCbnSelchangeCombo()
{
...
LRESULT CDlg_AT90USB::OnWizardNext()
{
	OnOK();
	return CMFCPropertyPage::OnWizardNext();

}
LRESULT CDlg_AT90USB::OnWizardBack()
{
	OnOK();
	return CMFCPropertyPage::OnWizardBack();
}

void CDlg_AT90USB::OnBnClickedEnable()
{
	CButton* Encheck = (CButton*)GetDlgItem(IDC_CHECK1);
	int state = Encheck->GetCheck();
	if (state == 1)
