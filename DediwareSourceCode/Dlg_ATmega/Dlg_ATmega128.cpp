// Dlg_ATmega128.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CDlg_ATmega128, CMFCPropertyPage)

CDlg_ATmega128::CDlg_ATmega128(struct chip_info_c *ChipInfo, unsigned char *RegisterBuff, unsigned long BuffLen, unsigned long *pDataLen, CWnd* pParent /*=NULL*/)
: CMFCPropertyPage(CDlg_ATmega128::IDD),
m_ChipInfo(ChipInfo),
...
CDlg_ATmega128::~CDlg_ATmega128()
{
}

void CDlg_ATmega128::DoDataExchange(CDataExchange* pDX)
{
	CMFCPropertyPage::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_CHECK1, Option_enable);
	DDX_Control(pDX, IDC_COMBO1, m_M103C);
	...
	DDX_Control(pDX, IDC_COMBO11, m_BODEN);
...
BEGIN_MESSAGE_MAP(CDlg_ATmega128, CMFCPropertyPage)
	ON_CBN_SELCHANGE(IDC_COMBO1, &CDlg_ATmega128::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO2, &CDlg_ATmega128::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO3, &CDlg_ATmega128::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO4, &CDlg_ATmega128::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO5, &CDlg_ATmega128::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO6, &CDlg_ATmega128::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO7, &CDlg_ATmega128::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO8, &CDlg_ATmega128::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO9, &CDlg_ATmega128::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO10, &CDlg_ATmega128::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO11, &CDlg_ATmega128::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO12, &CDlg_ATmega128::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO13, &CDlg_ATmega128::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO14, &CDlg_ATmega128::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO15, &CDlg_ATmega128::OnCbnSelchangeCombo)
	ON_BN_CLICKED(IDC_CHECK1, &CDlg_ATmega128::OnBnClickedEnable)
END_MESSAGE_MAP()


// Dlg_ATmega128 message handlers
BOOL CDlg_ATmega128::OnInitDialog()
{
	...
	m_ExFuse.SetWindowTextW(L"FD");
	m_HFuse.SetWindowTextW(L"99");
	m_LFuse.SetWindowTextW(L"E1");
	m_Lockbits.SetWindowTextW(L"FF");

	m_M103C.InsertString(m_M103C.GetCount(), L"0 = Enable ATmega103 compatibility mode");
	m_M103C.InsertString(m_M103C.GetCount(), L"1 = Disable ATmega103 compatibility mode");
	...
	m_OCDEN.InsertString(m_OCDEN.GetCount(), L"0 = Enable OCD");
	m_OCDEN.InsertString(m_OCDEN.GetCount(), L"1 = Disable OCD");
	m_JTAGEN.InsertString(m_JTAGEN.GetCount(), L"0 = Enable JTAG");
	m_JTAGEN.InsertString(m_JTAGEN.GetCount(), L"1 = Disable JTAG");
	...
	m_BOOTSZ.InsertString(m_BOOTSZ.GetCount(), L"00 = Boot Size 4096 words");
	m_BOOTSZ.InsertString(m_BOOTSZ.GetCount(), L"01 = Boot Size 2048 words");
	m_BOOTSZ.InsertString(m_BOOTSZ.GetCount(), L"10 = Boot Size 1024 words");
	m_BOOTSZ.InsertString(m_BOOTSZ.GetCount(), L"11 = Boot Size 512 words");
...
BOOL CDlg_ATmega128::OnSetActive()
{
	if (*m_pDataLen != NULL)
	{
		CButton* Encheck = (CButton*)GetDlgItem(IDC_CHECK1);
		Encheck->SetCheck(1);
		m_M103C.EnableWindow(TRUE);
		...
		//M103C
		if ((ExtendedFuse & 0x02) == 0x02)
			m_M103C.SetCurSel(1);
		else if ((ExtendedFuse & 0x02) == 0x00)
			m_M103C.SetCurSel(0);
		//WDTON
...
void CDlg_ATmega128::OnCbnSelchangeCombo()
{
	...
	//M103C
	ExtendedFuse &= 0xFD;
	if (m_M103C.GetCurSel() == 1)
		ExtendedFuse |= 0x02;
	else if (m_M103C.GetCurSel() == 0)
		ExtendedFuse |= 0x00;
...
LRESULT CDlg_ATmega128::OnWizardNext()
{
	OnOK();
	return CMFCPropertyPage::OnWizardNext();

}
LRESULT CDlg_ATmega128::OnWizardBack()
{
	OnOK();
	return CMFCPropertyPage::OnWizardBack();
}

void CDlg_ATmega128::OnBnClickedEnable()
{
	CButton* Encheck = (CButton*)GetDlgItem(IDC_CHECK1);
	int state = Encheck->GetCheck();
	if (state == 1)
