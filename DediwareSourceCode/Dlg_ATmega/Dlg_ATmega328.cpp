// Dlg_ATmega328.cpp : implementation file
//
...
// Dlg_ATmega328 dialog

IMPLEMENT_DYNAMIC(CDlg_ATmega328, CMFCPropertyPage)

CDlg_ATmega328::CDlg_ATmega328(struct chip_info_c *ChipInfo, unsigned char *RegisterBuff, unsigned long BuffLen, unsigned long *pDataLen, CWnd* pParent /*=NULL*/)
: CMFCPropertyPage(CDlg_ATmega328::IDD),
m_ChipInfo(ChipInfo),
...
CDlg_ATmega328::~CDlg_ATmega328()
{
}

void CDlg_ATmega328::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO7, m_BOOTSZ);
	DDX_Control(pDX, IDC_COMBO8, m_BOOTRST);
	DDX_Control(pDX, IDC_COMBO9, m_CKDIV8);
	DDX_Control(pDX, IDC_COMBO10, m_CKOUT);
	DDX_Control(pDX, IDC_COMBO11, m_CKSEL);
	DDX_Control(pDX, IDC_COMBO12, m_BLBB);
	DDX_Control(pDX, IDC_COMBO13, m_BLBA);
	DDX_Control(pDX, IDC_COMBO14, m_LOCK);
	DDX_Control(pDX, IDC_COMBO15, m_CFD);
...
BEGIN_MESSAGE_MAP(CDlg_ATmega328, CMFCPropertyPage)
	ON_CBN_SELCHANGE(IDC_COMBO1, &CDlg_ATmega328::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO2, &CDlg_ATmega328::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO3, &CDlg_ATmega328::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO4, &CDlg_ATmega328::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO5, &CDlg_ATmega328::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO6, &CDlg_ATmega328::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO7, &CDlg_ATmega328::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO8, &CDlg_ATmega328::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO9, &CDlg_ATmega328::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO10, &CDlg_ATmega328::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO11, &CDlg_ATmega328::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO12, &CDlg_ATmega328::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO13, &CDlg_ATmega328::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO14, &CDlg_ATmega328::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO15, &CDlg_ATmega328::OnCbnSelchangeCombo)
	ON_BN_CLICKED(IDC_CHECK1, &CDlg_ATmega328::OnBnClickedEnable)
END_MESSAGE_MAP()


// Dlg_ATmega328 message handlers

BOOL CDlg_ATmega328::OnInitDialog()
{
	CMFCPropertyPage::OnInitDialog();
	if (wcscmp(_T("ATmega328"), m_ChipInfo->description) == 0)
	{
		ExtendedFuse = 0xFF;
		m_ExFuse.SetWindowTextW(L"FF");
	}
	else if (wcscmp(_T("ATmega328PB"), m_ChipInfo->description) == 0)
	{
		ExtendedFuse = 0xF7;
		m_ExFuse.SetWindowTextW(L"F7");
	}
	HighFuse = 0xD9;
	LowFuse = 0x62;
	Lockbits = 0xFF;
	...
	//Extended Fuse
	m_CFD.InsertString(m_CFD.GetCount(), L"0 = Clock Failure Dectection Enable");
	m_CFD.InsertString(m_CFD.GetCount(), L"1 = Clock Failure Dectection Disable");
	...
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,0011 = Int.RC OSC:128KHz,StartupTime:6CK"); 		// 6 = 0x03
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,0011 = Int.RC OSC:128KHz,StartupTime:6CK+4.1ms"); 	// 7 = 0x13
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,0011 = Int.RC OSC:128KHz,StartupTime:6CK+65ms");  	// 8 = 0x23
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,0100 = Ext.Low.freq.crystal,StartupTime:1KCK"); 		// 9 = 0x04
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,0100 = Ext.Low.freq.crystal,StartupTime:1KCK+4.1ms"); 	// 10 = 0x14
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,0100 = Ext.Low.freq.crystal,StartupTime:1KCK+64ms");  	// 11 = 0x24
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,0101 = Ext.Low.freq.crystal,StartupTime:32KCK"); 		// 12 = 0x05
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,0101 = Ext.Low.freq.crystal,StartupTime:32KCK+4.1ms"); 	// 13 = 0x15
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,0101 = Ext.Low.freq.crystal,StartupTime:32KCK+64ms");  	// 14 = 0xx25
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,0110 = Ext.full-swing.crystal,StartupTime:258CK+4.1ms"); 		// 15 = 0x06
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,0110 = Ext.full-swing.crystal,StartupTime:258CK+65ms"); 	// 16 = 0x16
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,0110 = Ext.full-swing.crystal,StartupTime:1KCK");				// 17 = 0x26
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,0110 = Ext.full-swing.crystal,StartupTime:1KCK+4.1ms");  	// 18 = 0x36
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,0111 = Ext.full-swing.crystal,StartupTime:1KCK+65ms"); 		// 19 = 0x07
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,0111 = Ext.full-swing.crystal,StartupTime:16KCK"); 		// 20 = 0x17
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,0111 = Ext.full-swing.crystal,StartupTime:16KCK+4.1ms"); 	// 21 = 0x27
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,0111 = Ext.full-swing.crystal,StartupTime:16KCK+65ms"); 	// 22 = 0x37
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1000 = Ext.Crystal OSC:0.4-0.9MHz,StartupTime:258CK+4.1ms"); // 23 = 0x08
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1000 = Ext.Crystal OSC:0.4-0.9MHz,StartupTime:258CK+65ms"); 	// 24 = 0x18
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1000 = Ext.Crystal OSC:0.4-0.9MHz,StartupTime:1KCK"); 		// 25 = 0x28
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1000 = Ext.Crystal OSC:0.4-0.9MHz,StartupTime:1KCK+4.1ms");  // 26 = 0x38
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1001 = Ext.Crystal OSC:0.4-0.9MHz,StartupTime:1KCK+65ms"); 	// 27 = 0x09
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1001 = Ext.Crystal OSC:0.4-0.9MHz,StartupTime:16KCK"); 		// 28 = 0x19
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1001 = Ext.Crystal OSC:0.4-0.9MHz,StartupTime:16KCK+4.1ms"); // 29 = 0x29
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1001 = Ext.Crystal OSC:0.4-0.9MHz,StartupTime:16KCK+65ms");  // 30 = 0x39
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1010 = Ext.Crystal OSC:0.9-3.0MHz,StartupTime:258CK+4.1ms"); // 31 = 0x0A
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1010 = Ext.Crystal OSC:0.9-3.0MHz,StartupTime:258CK+65ms");  // 32 = 0x1A
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1010 = Ext.Crystal OSC:0.9-3.0MHz,StartupTime:1KCK");		   // 33 = 0x2A
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1010 = Ext.Crystal OSC:0.9-3.0MHz,StartupTime:1KCK+4.1ms");  // 34 = 0x3A
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1011 = Ext.Crystal OSC:0.9-3.0MHz,StartupTime:1KCK+65ms");	// 35 = 0x0B
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1011 = Ext.Crystal OSC:0.9-3.0MHz,StartupTime:16KCK"); 	 	// 36 = 0x1B
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1011 = Ext.Crystal OSC:0.9-3.0MHz,StartupTime:16KCK+4.1ms");	// 37 = 0x2B
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1011 = Ext.Crystal OSC:0.9-3.0MHz,StartupTime:16KCK+65ms");   // 38 = 0x3B
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1100 = Ext.Crystal OSC:3-8MHz,StartupTime:258CK+4.1ms");	 // 39 = 0x0C
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1100 = Ext.Crystal OSC:3-8MHz,StartupTime:258CK+65ms"); 	 // 40 = 0x1C
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1100 = Ext.Crystal OSC:3-8MHz,StartupTime:1KCK");	 		 // 41 = 0x2C
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1100 = Ext.Crystal OSC:3-8MHz,StartupTime:1KCK+4.1ms");    // 42 = 0x3C
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1101 = Ext.Crystal OSC:3-8MHz,StartupTime:1KCK+65ms");	 	// 43 = 0x0D
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1101 = Ext.Crystal OSC:3-8MHz,StartupTime:16KCK"); 	 	 	// 44 = 0x1D
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1101 = Ext.Crystal OSC:3-8MHz,StartupTime:16KCK+4.1ms");	 	// 45 = 0x2D
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1101 = Ext.Crystal OSC:3-8MHz,StartupTime:16KCK+65ms");   	// 46 = 0x3D
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1110 = Ext.Crystal OSC:>8MHz,StartupTime:258CK+4.1ms");		// 47 = 0x0E
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1110 = Ext.Crystal OSC:>8MHz,StartupTime:258CK+65ms"); 	 	// 48 = 0x1E
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1110 = Ext.Crystal OSC:>8MHz,StartupTime:1KCK");	 			// 49 = 0x2E
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1110 = Ext.Crystal OSC:>8MHz,StartupTime:1KCK+4.1ms");   		// 50 = 0x3E
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"00,1111 = Ext.Crystal OSC:>8MHz,StartupTime:1KCK+65ms");			// 51 = 0x0F
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"01,1111 = Ext.Crystal OSC:>8MHz,StartupTime:16KCK"); 	 		// 52 = 0x1F
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"10,1111 = Ext.Crystal OSC:>8MHz,StartupTime:16KCK+4.1ms");	 	// 53 = 0x2F
	m_CKSEL.InsertString(m_CKSEL.GetCount(), L"11,1111 = Ext.Crystal OSC:>8MHz,StartupTime:16KCK+65ms");   		// 54 = 0x3F
	//LOCK
...
BOOL CDlg_ATmega328::OnSetActive()
{
	if (*m_pDataLen != NULL)
	{
		Option_enable.SetCheck(1);
		if (wcscmp(_T("ATmega328"), m_ChipInfo->description) == 0)
		{
			m_CFD.EnableWindow(FALSE);
		}
...
void CDlg_ATmega328::OnCbnSelchangeCombo()
{
	...
	//CFD
	ExtendedFuse &= 0xF7;
	if (m_CFD.GetCurSel() == 1)
		ExtendedFuse |= 0x08;
	else if (m_CFD.GetCurSel() == 0)
		ExtendedFuse |= 0x00;
...
LRESULT CDlg_ATmega328::OnWizardNext()
{
	OnOK();
	return CMFCPropertyPage::OnWizardNext();

}
LRESULT CDlg_ATmega328::OnWizardBack()
{
	OnOK();
	return CMFCPropertyPage::OnWizardBack();
}

void CDlg_ATmega328::OnBnClickedEnable()
{
	if (Option_enable.GetCheck() == 1)
	{
		if (wcscmp(_T("ATmega328"), m_ChipInfo->description) == 0)
		{
			m_CFD.EnableWindow(FALSE);
	...
	else if (Option_enable.GetCheck() == 0)
	{
		m_CFD.EnableWindow(FALSE);
		m_BODLevel.EnableWindow(FALSE);
		m_RSTDISBL.EnableWindow(FALSE);
