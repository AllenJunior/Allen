// Dlg_ATmega8.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CDlg_ATmega8, CMFCPropertyPage)

CDlg_ATmega8::CDlg_ATmega8(struct chip_info_c *ChipInfo, unsigned char *RegisterBuff, unsigned long BuffLen, unsigned long *pDataLen, CWnd* pParent /*=NULL*/)
: CMFCPropertyPage(CDlg_ATmega8::IDD),
m_ChipInfo(ChipInfo),
...
CDlg_ATmega8::~CDlg_ATmega8()
{
}

void CDlg_ATmega8::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CDlg_ATmega8, CMFCPropertyPage)
	ON_CBN_SELCHANGE(IDC_COMBO1, &CDlg_ATmega8::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO2, &CDlg_ATmega8::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO3, &CDlg_ATmega8::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO4, &CDlg_ATmega8::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO5, &CDlg_ATmega8::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO6, &CDlg_ATmega8::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO7, &CDlg_ATmega8::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO8, &CDlg_ATmega8::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO9, &CDlg_ATmega8::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO10, &CDlg_ATmega8::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO11, &CDlg_ATmega8::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO12, &CDlg_ATmega8::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO13, &CDlg_ATmega8::OnCbnSelchangeCombo)
	ON_BN_CLICKED(IDC_CHECK1, &CDlg_ATmega8::OnBnClickedEnable)
END_MESSAGE_MAP()


// Dlg_ATmega8 message handlers
BOOL CDlg_ATmega8::OnInitDialog()
{
	...
	//Fuse High Byte
	if (wcscmp(_T("ATmega8"), m_ChipInfo->description) == 0)
	{
		GetDlgItem(IDC_STATIC2)->SetWindowTextW(L"RSTDISBL :");
	...
	if (wcscmp(_T("ATmega85X5"), m_ChipInfo->description) == 0)
	{
		GetDlgItem(IDC_STATIC2)->SetWindowTextW(L"S85X5C :");
		m_RSTDISBL.InsertString(m_RSTDISBL.GetCount(), L"0 = Enable compatibility mode");
		m_RSTDISBL.InsertString(m_RSTDISBL.GetCount(), L"1 = Disabled compatibility mode");
	}
...
void CDlg_ATmega8::OnCbnSelchangeCombo()
{
...
LRESULT CDlg_ATmega8::OnWizardNext()
{
	OnOK();
	return CMFCPropertyPage::OnWizardNext();

}
LRESULT CDlg_ATmega8::OnWizardBack()
{
	OnOK();
	return CMFCPropertyPage::OnWizardBack();
}

void CDlg_ATmega8::OnBnClickedEnable()
{
	CButton* Encheck = (CButton*)GetDlgItem(IDC_CHECK1);
	int state = Encheck->GetCheck();
	if (state == 1)
