...

{
	...
	std::string temp1_str, temp2_str, temp3_str, end_str;
	const char *end_point = "</ProjectData>";
	const char *rail1_cmp = "Chan #1";
	const char *rail2_cmp = "Chan #2";
	FILE *hFile = NULL;
	...
	do{
		...
		temp1_str.erase(0, 6);	//<Code>20</Code>
		str_length = temp1_str.length();
		temp1_str.erase(str_length - 7, str_length);
		addr = strtol(temp1_str.c_str(), NULL, 10);  // Dec to Hex
		//Next String 
		//Find data <IDAndCode>
		std::getline(input, temp3_str);
		str_length = temp3_str.length();
		temp3_str.erase(0, str_length - 20);
		str_length = temp3_str.length();
		temp3_str.erase(7, str_length);
		if ((strcmp(temp3_str.c_str(), rail1_cmp)) == 0)
			addr = (addr * 2) + 0x00;
		if ((strcmp(temp3_str.c_str(), rail2_cmp)) == 0)
			addr = (addr * 2) + 0x200;
		else if ((strcmp(temp3_str.c_str(), rail1_cmp)) != 0 && (strcmp(temp3_str.c_str(), rail2_cmp)) != 0)
			addr = (addr * 2);
		do {
		...
		memcpy(&vMemBuf.at(addr), &data, sizeof(data));
		checksum += data;
	} while (!feof(hFile));
end:
