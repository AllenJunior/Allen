// Dlg_Nano1X0BN.cpp : implementation file
//
...
// Dlg_Nano1X0BN dialog

IMPLEMENT_DYNAMIC(CDlg_Nano1X0BN, CMFCPropertyPage)

CDlg_Nano1X0BN::CDlg_Nano1X0BN(struct chip_info_c * ChipInfo, unsigned char *RegisterBuff, unsigned long BuffLen, unsigned long *pDataLen, CWnd* pParent /*=NULL*/)
: CMFCPropertyPage(CDlg_Nano1X0BN::IDD),
m_ChipInfo(ChipInfo),
...
CDlg_Nano1X0BN::~CDlg_Nano1X0BN()
{
}

void CDlg_Nano1X0BN::DoDataExchange(CDataExchange* pDX)
{
	CMFCPropertyPage::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_EDIT1, m_Cfg0);
	DDX_Control(pDX, IDC_EDIT2, m_Cfg1);
	DDX_Control(pDX, IDC_COMBO1, m_CWDT_EN);
...
BEGIN_MESSAGE_MAP(CDlg_Nano1X0BN, CMFCPropertyPage)
	ON_CBN_SELCHANGE(IDC_COMBO1, &CDlg_Nano1X0BN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO2, &CDlg_Nano1X0BN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO3, &CDlg_Nano1X0BN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO4, &CDlg_Nano1X0BN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO5, &CDlg_Nano1X0BN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO6, &CDlg_Nano1X0BN::OnCbnSelchangeCombo)
	ON_EN_CHANGE(IDC_EDIT3, &CDlg_Nano1X0BN::OnEnChangeEdit3)
END_MESSAGE_MAP()


// Dlg_Nano1X0BN message handlers

BOOL CDlg_Nano1X0BN::OnInitDialog()
{
	...
	m_CWDT_EN.InsertString(m_CWDT_EN.GetCount(), L"0 = Enable");
	m_CWDT_EN.InsertString(m_CWDT_EN.GetCount(), L"1 = Disable");
...
BOOL CDlg_Nano1X0BN::OnSetActive()
{
	CString CFG0;
	CString CFG1;
	if (*m_pDataLen != NULL)
...
void CDlg_Nano1X0BN::OnCbnSelchangeCombo()
{
	CString strTemp;
	CString strTemp2;
	Val_Cfg0 = 0xFFFFFFFF;
	...
	//Watchdog timer
	Val_Cfg0 &= 0x7FFFFFFF;
	if (m_CWDT_EN.GetCurSel() == 1)
		Val_Cfg0 |= 0x80000000;
	else if (m_CWDT_EN.GetCurSel() == 0)
		Val_Cfg0 |= 0x00;
...
LRESULT CDlg_Nano1X0BN::OnWizardNext()
{
	OnOK();
	return CMFCPropertyPage::OnWizardNext();
}
LRESULT CDlg_Nano1X0BN::OnWizardBack()
{
	OnOK();
...
void CDlg_Nano1X0BN::OnEnChangeEdit3()
{
	CString Addr;
