// Dlg_NUC120DN.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CDlg_NUC120DN, CMFCPropertyPage)

CDlg_NUC120DN::CDlg_NUC120DN(struct chip_info_c * ChipInfo, unsigned char *RegisterBuff, unsigned long BuffLen, unsigned long *pDataLen, CWnd* pParent /*=NULL*/)
: CMFCPropertyPage(CDlg_NUC120DN::IDD),
m_ChipInfo(ChipInfo),
...
CDlg_NUC120DN::~CDlg_NUC120DN()
{
}

void CDlg_NUC120DN::DoDataExchange(CDataExchange* pDX)
{
	CMFCPropertyPage::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_EDIT1, m_cfg0);
	DDX_Control(pDX, IDC_EDIT2, m_cfg1);
	DDX_Control(pDX, IDC_COMBO1, m_lock);
	DDX_Control(pDX, IDC_COMBO2, m_IOinit);
	DDX_Control(pDX, IDC_COMBO3, m_clkSource);
	DDX_Control(pDX, IDC_COMBO4, m_XTINOUT);
	DDX_Control(pDX, IDC_COMBO5, m_WdtEn);
	DDX_Control(pDX, IDC_COMBO6, m_WDTPD);
	DDX_Control(pDX, IDC_COMBO7, m_boot);
	DDX_Control(pDX, IDC_COMBO8, m_BroDet);
	DDX_Control(pDX, IDC_COMBO9, m_BroVolt);
	DDX_Control(pDX, IDC_COMBO10, m_BroRst);
	DDX_Control(pDX, IDC_COMBO11, m_DataFlash);
}


BEGIN_MESSAGE_MAP(CDlg_NUC120DN, CMFCPropertyPage)
	ON_CBN_SELCHANGE(IDC_COMBO1, &CDlg_NUC120DN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO2, &CDlg_NUC120DN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO3, &CDlg_NUC120DN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO4, &CDlg_NUC120DN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO5, &CDlg_NUC120DN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO6, &CDlg_NUC120DN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO7, &CDlg_NUC120DN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO8, &CDlg_NUC120DN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO9, &CDlg_NUC120DN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO10, &CDlg_NUC120DN::OnCbnSelchangeCombo)
	ON_CBN_SELCHANGE(IDC_COMBO11, &CDlg_NUC120DN::OnCbnSelchangeCombo)
	ON_EN_CHANGE(IDC_EDIT3, &CDlg_NUC120DN::OnEnChangeEdit3)
END_MESSAGE_MAP()


// Dlg_NUC120DN message handlers
BOOL CDlg_NUC120DN::OnInitDialog()
{
	...
	m_clkSource.InsertString(m_clkSource.GetCount(), L"000 = External XTAL");
	m_clkSource.InsertString(m_clkSource.GetCount(), L"111 = Internal OSC(22.1184MHz)");
	m_XTINOUT.InsertString(m_XTINOUT.GetCount(), L"0 = GPIO");
	m_XTINOUT.InsertString(m_XTINOUT.GetCount(), L"1 = 4~24MHz External Crystal Pin");
	m_WdtEn.InsertString(m_WdtEn.GetCount(), L"0 = Enable");
	m_WdtEn.InsertString(m_WdtEn.GetCount(), L"1 = Enable");
	...
	m_BroVolt.InsertString(m_BroVolt.GetCount(), L"10 = 3.7V");
	m_BroVolt.InsertString(m_BroVolt.GetCount(), L"11 = 4.4V");
	...
	m_DataFlash.InsertString(m_DataFlash.GetCount(), L"0 = Enable");
	m_DataFlash.InsertString(m_DataFlash.GetCount(), L"1 = Disable");

	m_lock.SetCurSel(1);
	m_IOinit.SetCurSel(1);
	m_clkSource.SetCurSel(1);
	m_XTINOUT.SetCurSel(1);
	m_WdtEn.SetCurSel(1);
	m_WDTPD.SetCurSel(1);
...
BOOL CDlg_NUC120DN::OnSetActive()
{
	CString CFG0;
	CString CFG1;
	if (*m_pDataLen != NULL)
...
void CDlg_NUC120DN::OnCbnSelchangeCombo()
{
	...
	Val_Cfg0 &= 0xF7FFFFFF;
	if (m_XTINOUT.GetCurSel() == 1)
		Val_Cfg0 |= 0x08000000;
	else if (m_XTINOUT.GetCurSel() == 0)
		Val_Cfg0 |= 0x00;
	strTemp.Format(_T("%08X"), Val_Cfg0);
	m_cfg0.SetWindowTextW(strTemp);
	//WDT Clock
...
LRESULT CDlg_NUC120DN::OnWizardNext()
{
	OnOK();
	return CMFCPropertyPage::OnWizardNext();
}
LRESULT CDlg_NUC120DN::OnWizardBack()
{
	OnOK();
	return CMFCPropertyPage::OnWizardBack();
}
void CDlg_NUC120DN::OnEnChangeEdit3()
{
	CString Addr;
